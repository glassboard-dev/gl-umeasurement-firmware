...
int main(void)
{
    ...
    if (kStatus_Success == I2C_MasterStart(EXAMPLE_I2C_MASTER, I2C_MASTER_SLAVE_ADDR_7BIT, kI2C_Write))
    {
        commandAddr = 0x08; // RelativeStateOfCharge()

        reVal = I2C_MasterWriteBlocking(EXAMPLE_I2C_MASTER, &commandAddr, 1, kI2C_TransferNoStopFlag);
        if (reVal != kStatus_Success)
        {
            return -1;
        }
    ...
    for (uint32_t i = 0U; i < I2C_DATA_LENGTH; i++)
    {
        ...
        PRINTF("x%02X  ", g_master_rxBuff[i]);
    }
    PRINTF("\r\n\r\n");
